...
call mvn jtest:jtest -Djtest.publish=true -Djtest.config="jtest.dtp://PCI DSS 3.2" -Dmaven.test.skip=true -Dmaven.test.failure.ignore=true -Dproperty.dtp.project=%DTP_PROJECT% -Djtest.showsettings=true -Djtest.report=%REPORT_DIR%\jtest-sa-pci_dss -Dproperty.console.verbosity.level=high -Dproperty.build.id=%BUILD_ID% -Djacoco.skip=true > %LOG_DIR%\jtest-pci-dss-%RUN_TIME%.log 2>&1
